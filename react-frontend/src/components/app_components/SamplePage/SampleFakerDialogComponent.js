...
import sampleFakerFactory from "./sampleFakerFactory";
...
const SampleFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sampleFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sample").create(elem));
...
export default connect(mapState, mapDispatch)(SampleFakerDialogComponent);
